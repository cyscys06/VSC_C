...
int main()
{
    ...
    vector<vector<int>> G(N); // 무방향/희소 그래프이므로, N * N인 2차원 벡터 만들기(노드를 기준으로 탐색)
                              // 또는 vector<int> G[N]으로 각각의 인덱스가 벡터인 배열(즉 2차원인 배열)도 만들수 있음
    for (int i = 0; i < M; i++)
    {
        cin >> u >> v;
    ...
    for (int i = 0; i < N; i++) 
    {
        ...
        count++; // 비연결 노드 나올때마다 컴포넌트 개수 1 증가시키기
        
        queue<int> q;
        q.push(i); // 큐에 노드 번호 푸쉬
        vis[i] = true; // 방문 노드는 1로 표시
        ...
        while(!q.empty()) // q 안에 뭐가 있을때(즉 하나의 컴포넌트 안의 모든 연결된 노드들을 탐색)
        {
            ...
            for (auto next : G[cur]) // cur: 간선의 시작정점 또는 연결된 정점
            {
                if (vis[next] == false)
                {
                    q.push(next);
